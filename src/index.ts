...
//
const listaCompromissos:compromisso[]=[
    {
        Titulo:"SHAZAM!",
        Data_Hora:new Date(),
...
function adicionarCompromisso(compromissos:compromisso[]):void{
    ...
    console.log("Novo Compromisso adicionado:",compromissosOrdenados.map(compromisso=>({
        ...compromisso,
        Data_Hora:compromisso.Data_Hora.toLocaleString()
    })))
};
//
function listarTodosCompromissos():void{
    const compromissosOrdenados = listaCompromissos.sort((a, b) => a.Data_Hora.getTime() - b.Data_Hora.getTime());
    
    console.log(compromissosOrdenados.map(compromisso=>({
        ...compromisso,
        Data_Hora:compromisso.Data_Hora.toLocaleString()
    })))
};
//
function listarStatusCompromissos(status:Status):void{
...
//
function editarCompromisso(Titulo:string, dadosNovos:Partial<compromisso>):void{
    const localizar=listaCompromissos.findIndex(compromisso=>compromisso.Titulo===Titulo);

    if(localizar!==-1){
        listaCompromissos[localizar]={
            ...listaCompromissos[localizar],
            ...dadosNovos
        }
        const compromissoAtualizado = {
            ...listaCompromissos[localizar],
            Data_Hora: listaCompromissos[localizar].Data_Hora.toLocaleString()
        };
...
//
function deletarComprisso(Titulo:string):void{
